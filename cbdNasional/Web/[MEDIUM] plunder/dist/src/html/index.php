...
class Sanitizer
{
    ...
    protected function cleanTags($source)
    {
        ...
        while ($tagOpenStart !== false) {
            ...
            list($tagName, $isCloseTag) = $this->parseTag($currentTag);
            ...
            $attrSet = $this->parseAttributes($currentTag);
            if ($this->isAllowedTag($tagName)) {
                $preTag .= $this->renderTag($tagName, $isCloseTag, $attrSet, $fromTagOpen);
            }
            $postTag = mb_substr($postTag, ($tagLength + 2));
            $tagOpenStart = mb_strpos($postTag, '<');
        }
    ...
    protected function parseTag($currentTag)
    {
        if (mb_substr($currentTag, 0, 1) === '/') {
            list($tagName) = explode(' ', $currentTag);
            return [mb_substr($tagName, 1), true];
        }
    ...
    {
        return preg_match('/^[a-z][a-z0-9]*$/i', $tagName) && $tagName;
    }

    protected function parseAttributes($tagLeft)
    {
        $attrSet = [];
    ...
    protected function isAllowedTag($tagName)
    {
        return in_array(strtolower($tagName), $this->allowedTags);
    }

    protected function renderTag($tagName, $isCloseTag, array $attrSet, $fromTagOpen)
    {
        if ($isCloseTag) {
            return '</' . $tagName . '>';
        }
    ...
    protected function cleanAttributes(array $attrSet)
    {
        ...
        $count = count($attrSet);
        for ($i = 0; $i < $count; $i++) {
            $attr = $this->cleanAttribute($attrSet[$i]);
            if ($attr !== null) {
                $newSet[] = $attr;
            }
        }
    ...
    protected function cleanAttribute($attr)
    {
        if (!$attr) {
            return null;
        }

        $attrSubSet = $this->splitAttribute($attr);
        $attrSubSet[0] = $this->normalizeAttributeName($attrSubSet[0]);
        ...
        $attrSubSet[1] = $this->normalizeAttributeValue($attrSubSet[1]);
        if (static::checkAttribute($attrSubSet)) {
            return null;
        }
        if (!$this->isAllowedAttribute($attrSubSet[0])) {
            return null;
        }

        return $this->renderAttribute($attrSubSet[0], $attrSubSet[1]);
    }

    protected function splitAttribute($attr)
    {
        $attrSubSet = explode('=', trim($attr), 2);
    ...
    protected function normalizeAttributeName($attrName)
    {
        ...
        $attrName = html_entity_decode($attrName, $quoteStyle, 'UTF-8');
        $attrName = preg_replace('/^[\pZ\pC]+|[\pZ\pC]+$/u', '', $attrName);
        $attrName = preg_replace('/\s+/u', '', $attrName);
        $attrName = $this->stripBlacklistedEntities($attrName);
        return preg_replace('/[^\p{L}\p{N}\-\s]/u', '', $attrName);
    }

    protected function normalizeAttributeValue($attrValue)
    {
        $attrValue = $this->stripBlacklistedEntities($attrValue);
        $attrValue = trim($attrValue);
        $attrValue = str_replace('&#', '', $attrValue);
        $attrValue = preg_replace('/[\n\r]/', '', $attrValue);
        $attrValue = str_replace('"', '', $attrValue);
        if ((mb_substr($attrValue, 0, 1) == "'") && (mb_substr($attrValue, (\strlen($attrValue) - 1), 1) == "'")) {
            $attrValue = mb_substr($attrValue, 1, (\strlen($attrValue) - 2));
        }
        return stripslashes($attrValue);
    }

    protected function stripBlacklistedEntities($value)
    {
        foreach ($this->blacklistEnt as $blockedChar) {
            $value = str_ireplace($blockedChar, '', $value);
        }
        return $value;
    }

    protected function isAllowedAttribute($attrName)
    {
        return in_array(strtolower($attrName), $this->allowedAttrs);
    }

    protected function renderAttribute($attrName, $attrValue)
    {
...
try {
    ...
    $input = new Sanitizer();
    echo $input->purify($_REQUEST['html'] ?? 'hmm');
} catch (\Throwable $th) {
    echo '';
}
